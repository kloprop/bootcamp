...
public class JavaQuest16 {
  ...
  public static int [] sumToZero(int n) {
    int [] nums = new int [n];
    int sum = 0;
    for (int i = 1 ;i< n; i++){ // if i = 0, when n = 2, 0 is used twice 
      nums[ i-1 ] = i; // i-1 makes sure the eles are changed from the start, while remain the last to be replaced by the sum
      sum+=i;
    }
    nums[n-1] = sum*-1;
    return nums;
